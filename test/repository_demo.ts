 ...
 * リポジトリの使用例を示すデモ
 */
async function repositoryDemo() {
    console.log('===== IdressRepository デモ =====');
    
    // サンプルデータの作成
    const sampleText = `A：１：名前：ルウシィ・紅葉
オーナー：サンプルオーナー
タイプ：キャラクター
スケール：３
攻撃：１：ファイアボール：炎の球を放つ
...
適用勲章１：勇気の勲章：攻撃力+1：https://example.com/medal1`;
    ...
    await memoryRepositoryExample(memoryRepo, sampleData);
}
 ...
 */
async function memoryRepositoryExample(repo: IdressRepository, sampleData: IdressData) {
    try {
        // 1. データの作成
        ...
        console.log(`  作成したデータの名前: '${extractName(createdData)}'`);
